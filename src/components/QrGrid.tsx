...
export default function QrGrid({ codes, qrOptions }: QrGridProps) {
  const downloadQrCode = (code: string, index: number) => {
    ...
    qrCode.download({ name: `qr_code_${index + 1}`, extension: qrOptions.fileExtension });
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Here are your generated QR codes. Use the sidebar to download them in different formats.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
        {codes.map((code, i) => (
          <Card key={i} className="flex flex-col items-center justify-center p-4 gap-3">
            <div id={`qr-code-container-${i}`}>
              <QrCodeItem code={code} qrOptions={qrOptions} />
            </div>
            <p className="text-xs text-center break-all">{code}</p>
            <Button variant="outline" size="sm" onClick={() => downloadQrCode(code, i)}>
